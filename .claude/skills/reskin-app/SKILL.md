...
5. Then go through **every** CSS Module file listed above, one by one, and bring each in line with the new theme. This is what makes the result cohesive rather than a re-tinted version of the old design (see `lesson-2.8-pixel` for a worked example of this level of pass). In each file, look for:
